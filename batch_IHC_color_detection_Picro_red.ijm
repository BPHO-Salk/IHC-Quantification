// Batch IHC quantification - extract Picro red staining

// An ImageJ/FIJI macro script for batch extracting Picro red staining using Immunohistochemistry (IHC) Image Analysis Toolbox
...
open(filelist[0]);      
run("IHC Toolbox", "");
waitForUser("User selects custom color detection model", "Select \"Read User Model\" for Picro red (red), then click OK.")
close()
for (i=0; i< filelist.length; i++) {
	...
	if (endsWith(filelist[i], ".tif") || endsWith(filelist[i], ".tiff"))  {
		...
		analyzedName = replace(replace(filelist[i],".tiff","_Sirius_red.tiff"),".tif","_Sirius_red.tif");
...
resultName = "results-Sirius_Red.xls";
