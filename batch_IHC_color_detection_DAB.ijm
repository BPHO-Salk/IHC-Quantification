// Batch IHC quantification - extract DAB staining
...
open(filelist[0]);      
run("IHC Toolbox", "");
waitForUser("User selects custom color detection model", "Select \"Read User Model\" for DAB (brown), then click OK.")
close()
for (i=0; i< filelist.length; i++) {
	...
	if (endsWith(filelist[i], ".tif") || endsWith(filelist[i], ".tiff"))  {
		...
		analyzedName = replace(replace(filelist[i],".tiff","_DAB.tiff"),".tif","_DAB.tif");
